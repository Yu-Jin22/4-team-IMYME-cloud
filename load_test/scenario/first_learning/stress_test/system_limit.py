...
테스트 설정:
  - 동시 사용자: 0 → 200명 (공격적으로 증가)
  - 증가율: 초당 5명 (빠르게 부하 증가)
  - 실행 시간: 수동 중단 (시스템 장애 발생 시 또는 최대 사용자 도달 시)
  - 특징: 시스템 한계까지 부하를 가하여 장애 발생 지점 확인
...
실행 방법:
  ...
  locust -f load_test/scenario/first_learning/stress_test/system_limit.py \\
    --host=http://3.39.31.181:8080 \\
    --users 200 --spawn-rate 5 --headless
...
class FirstLearningScenario(SequentialTaskSet):
    ...
    def step5_create_attempt(self):
        ...
        with self.client.post(
            ...
            json=payload,
            headers=self.get_headers(),
            name="[Stress-Limit] 5. Attempt 생성",
            catch_response=True,
        ) as response:
            if response.status_code == 201:
                data = response.json()
...
class SystemLimitUser(HttpUser):
    """시스템 한계 테스트 사용자"""
    wait_time = between(1, 3)  # 빠른 요청으로 더 강한 부하
